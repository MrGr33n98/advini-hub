...
export function LawyerCard({ lawyer }: { lawyer: Lawyer }) {
  const photoUrl = lawyer.photo_url || `${import.meta.env.BASE_URL}images/lawyer-placeholder-1.png`;
  ...
  const responseTime = getResponseTimeBadge(lawyer.avg_rating);
        ...
        <div className="text-sm">
          {lawyer.hourly_rate_min ? (
            <div>
              ...
              <span className="font-bold text-foreground">R$ {lawyer.hourly_rate_min}</span>
            </div>
          ) : (
            <div>
              <span className="text-xs text-muted-foreground block">Experiência</span>
              <span className="font-semibold text-foreground">{lawyer.years_experience} anos</span>
            </div>
          )}
        </div>
